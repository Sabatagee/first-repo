...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	allRunningCalories := ((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return allRunningCalories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	speedInSec := meanSpeed(action, duration) * kmhInMsec // перевод скорости в м/c
	doubleSpeed := math.Pow(speedInSec, 2)                // возведение средней скорости в квадрат
	allWalkingCalories := ((walkingCaloriesWeightMultiplier*weight + (doubleSpeed/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return allWalkingCalories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	allSwimmingCalories := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return allSwimmingCalories
}
